...
**A**, since the devnet feed account is live and fresh at the time of checking; **C** if a 24-hour cadence measurement (P08's first day, logged per tick) shows gaps longer than the policy's `max_mark_age_slots`. In both cases P08 starts immediately with `MARK_SOURCE=onchain` reading `7UVimffx…` over RPC, because the paper runner needs a slot-stamped price today and Hermes cannot provide one without a key. B is the last resort and requires Kunal to open a Pyth Pro or provider account first.
