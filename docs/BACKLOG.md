...
| Keyless `logsSubscribe` is proven to deliver, but the **subscription limit and drop behaviour under load are unmeasured** on both keyless endpoints | P09 can start; the limit only bites at scale | P09 measures it, or the RPC key removes the question |
